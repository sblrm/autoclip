...
import { api, type Artifact, type Clip, type FaceTrack, type Job, type OnboardingSnapshot, type Project, type ProjectDetail, type RuntimeHealth, type StudioClient } from "./api";
...
const copy = {
  id: {
    ...
    trackerChanged: "Mesin tracking berubah. Kandidat dan kunci subjek lama dihapus agar wajah tidak berpindah diam-diam. Deteksi ulang, lalu kunci subjek baru.",
    selected: "Terkunci",
    gaps: "Gap pelacakan",
    exportHistory: "Riwayat ekspor",
    previewStale: "Pratinjau lama. Buat ulang sebelum menyetujui atau mengekspor.",
    ready: "siap review",
  },
  en: {
    ...
    trackerChanged: "The tracking engine changed. Old candidates and the subject lock were cleared to prevent a silent face switch. Detect again, then lock a new subject.",
    selected: "Locked",
    gaps: "Tracking gaps",
    exportHistory: "Export history",
    previewStale: "Preview is stale. Rebuild it before approval or export.",
    ready: "ready to review",
  },
} as const;

type AppCopy = { [Key in keyof (typeof copy)["id"]]: string };
...
export function App({ client = api }: AppProps) {
  ...
  if (route.kind === "home") {
    return <HomeRoute client={client} onOpenProject={(projectId) => navigate({ kind: "project", projectId })} onOpenSettings={() => navigate({ kind: "settings" })} />;
  }
  if (route.kind === "settings") {
    return <PerformanceSettings client={client} locale="id" onOpenHome={() => navigate({ kind: "home" })} />;
  }
  return <StudioEditor
    client={client}
...
function RoutePlaceholder({ title, body, onOpenSettings }: { title: string; body: string; onOpenSettings: () => void }) {
  return <main className="grid min-h-screen place-items-center bg-[#10100f] px-6 text-stone-100"><section className="max-w-xl border border-white/8 bg-[#151513] p-8"><p className="font-mono text-[10px] tracking-[.2em] text-[#f5662c]">AUTOCLIP / LOCAL STUDIO</p><h1 className="mt-4 text-4xl font-semibold tracking-[-.05em]">{title}</h1><p className="mt-4 leading-7 text-stone-400">{body}</p><button className="action-secondary mt-7" onClick={onOpenSettings}>{title === "Performa" ? "Kembali ke Home" : "Buka Settings"}</button></section></main>;
}

function HomeRoute({ client, onOpenProject, onOpenSettings }: { client: StudioClient; onOpenProject: (projectId: string) => void; onOpenSettings: () => void }) {
  const [snapshot, setSnapshot] = useState<OnboardingSnapshot | null>(null);
  const [projects, setProjects] = useState<Project[]>([]);
  const [issue, setIssue] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([client.getOnboarding(), client.listProjects()])
      .then(([nextSnapshot, nextProjects]) => {
  ...
  if (issue) return <RoutePlaceholder title="AutoClip Studio" body={issue} onOpenSettings={onOpenSettings} />;
  if (!snapshot) return <main className="grid min-h-screen place-items-center bg-[#10100f] font-mono text-xs tracking-[.2em] text-stone-500">MEMUAT STUDIO…</main>;
...
function StudioEditor({ client, projectId, onOpenProject, onOpenSettings }: Required<AppProps> & { projectId: string; onOpenProject: (projectId: string) => void; onOpenSettings: () => void }) {
  ...
  const [stalePreviewClipIds, setStalePreviewClipIds] = useState<Set<string>>(() => new Set());
  const stopWatchingJob = useRef<(() => void) | null>(null);
  const t = copy[locale];
  ...
  useEffect(() => () => {
    stopWatchingJob.current?.();
    stopWatchingJob.current = null;
  }, []);
  ...
  const previewStale = activeClip ? stalePreviewClipIds.has(activeClip.id) : false;

  const runJob = useCallback(async (start: () => Promise<{ job_id: string }>, freshPreviewClipId?: string) => {
    setBusy(true);
    setIssue(null);
    try {
      ...
      let cancel = () => { finishedSynchronously = true; };
      const nextCancel = client.watchJob(created.job_id, (event) => {
        setJob(event);
        if (!terminal && ["completed", "failed", "interrupted"].includes(event.stage)) {
          terminal = true;
          cancel();
          stopWatchingJob.current = null;
          const detailRefresh = projectId ? refreshDetail(projectId) : Promise.resolve(null);
          Promise.all([detailRefresh, refreshLibrary()]).then(([refreshedDetail]) => {
            if (
              freshPreviewClipId
              && event.stage === "completed"
              && refreshedDetail
              && previewMatchesSelection(refreshedDetail, freshPreviewClipId)
            ) {
              setStalePreviewClipIds((current) => {
                const next = new Set(current);
                next.delete(freshPreviewClipId);
                return next;
              });
            }
          }).catch((error: Error) => setIssue(error.message)).finally(() => setBusy(false));
        }
      });
      cancel = nextCancel;
      stopWatchingJob.current = nextCancel;
      if (finishedSynchronously) nextCancel();
    } catch (error) {
  ...
  const accelerationChanged = useCallback(async () => {
    if (!activeProjectId) return;
    if (activeClipId) {
      setStalePreviewClipIds((current) => new Set(current).add(activeClipId));
    }
    await refreshDetail(activeProjectId);
  }, [activeClipId, activeProjectId, refreshDetail]);
  ...
  return (
    <main className="min-h-screen bg-[#10100f] text-stone-100 selection:bg-[#f5662c] selection:text-black">
      <div className="studio-grid min-h-screen">
        <Sidebar projects={projects} activeProjectId={activeProjectId} health={health} t={t} locale={locale} onLocale={() => setLocale((value) => value === "id" ? "en" : "id")} onSelect={onOpenProject} onSettings={onOpenSettings} />
        <section className="min-w-0 border-x border-white/8 bg-[#151513]">
          ...
          {detail ? (
            <Editor detail={detail} clip={activeClip} job={job} busy={busy} t={t} onSelectClip={setActiveClipId} onPatch={updateClip} onPreview={() => activeClip && runJob(() => client.createPreview(activeClip.id), activeClip.id)} onApprove={async () => { if (activeClip && activeProjectId) { await client.approve(activeClip.id); await refreshDetail(activeProjectId); } }} onExport={() => activeClip && runJob(() => client.exportClip(activeClip.id))} />
          ) : (
            <ImportScreen t={t} busy={busy} onImport={importSource} />
          )}
        </section>
        <Inspector client={client} locale={locale} detail={detail} clip={activeClip} health={health} job={job} busy={busy} previewStale={previewStale} t={activeClip && !activeClip.tracking_resolution ? { ...t, makePreview: t.detectFaces, previewStale: t.trackerChanged } : t} onAccelerationChanged={accelerationChanged} onPatch={updateClip} onPreview={() => activeClip && runJob(() => client.createPreview(activeClip.id), activeClip.id)} onApprove={async () => { if (activeClip && activeProjectId) { await client.approve(activeClip.id); await refreshDetail(activeProjectId); } }} onExport={() => activeClip && runJob(() => client.exportClip(activeClip.id))} />
      </div>
...
function Sidebar({ projects, activeProjectId, health, t, locale, onLocale, onSelect, onSettings }: { projects: Project[]; activeProjectId: string | null; health: RuntimeHealth | null; t: AppCopy; locale: Locale; onLocale: () => void; onSelect: (id: string) => void; onSettings: () => void }) {
  return <aside className="hidden min-h-screen flex-col bg-[#10100f] lg:flex">
    ...
    <div className="mt-auto border-t border-white/8 p-4"><button className="mb-4 flex items-center gap-2 font-mono text-[10px] text-stone-400 hover:text-white" onClick={onSettings}><GearSix size={14} />SETTINGS</button><div className="flex items-center justify-between font-mono text-[10px] text-stone-500"><span>LOCAL ENGINE</span><span className={health?.face_tracking.available ? "text-[#f5662c]" : "text-red-400"}>{health?.face_tracking.available ? "ONLINE" : "CHECK"}</span></div><div className="mt-3 h-px bg-stone-800"><div className="h-px w-2/3 bg-[#f5662c]" /></div></div>
  </aside>;
}

function Topbar({ detail, health, t, onAnalyze }: { detail: ProjectDetail | null; health: RuntimeHealth | null; t: AppCopy; onAnalyze: () => void }) {
...
function ImportScreen({ t, busy, onImport }: { t: AppCopy; busy: boolean; onImport: (source: File | string) => Promise<void> }) {
...
function Editor({ detail, clip, job, busy, t, onSelectClip, onPatch, onPreview, onApprove, onExport }: { detail: ProjectDetail; clip: Clip | null; job: Job | null; busy: boolean; t: AppCopy; onSelectClip: (id: string) => void; onPatch: (patch: Parameters<StudioClient["patchClip"]>[1]) => Promise<void>; onPreview: () => void; onApprove: () => void; onExport: () => void }) {
...
function Inspector({ client, locale, detail, clip, health, job, busy, previewStale, t, onAccelerationChanged, onPatch, onPreview, onApprove, onExport }: { client: StudioClient; locale: Locale; detail: ProjectDetail | null; clip: Clip | null; health: RuntimeHealth | null; job: Job | null; busy: boolean; previewStale: boolean; t: AppCopy; onAccelerationChanged: () => Promise<void>; onPatch: (patch: Parameters<StudioClient["patchClip"]>[1]) => Promise<void>; onPreview: () => void; onApprove: () => void; onExport: () => void }) {
  ...
  return <aside className="inspector-panel"><div className="flex items-center justify-between border-b border-white/8 px-5 py-5"><div><p className="font-mono text-[10px] uppercase tracking-[.15em] text-stone-500">{t.inspector}</p><p className="mt-1 text-sm text-stone-200">{clip?.tracking_status ?? "—"}</p></div><HealthDialog health={health} t={t} /></div>{clip && detail ? <div className="space-y-7 p-5"><div><label className="studio-label">{t.title}</label><input className="studio-input" defaultValue={clip.title} onBlur={(event) => onPatch({ title: event.currentTarget.value })} /></div><div className="grid grid-cols-2 gap-3"><NumberField label={t.in} value={clip.start_time} onCommit={(start_time) => onPatch({ start_time })} /><NumberField label={t.out} value={clip.end_time} onCommit={(end_time) => onPatch({ end_time })} /></div><div><button className="action-secondary w-full justify-center" onClick={() => setShowAdvanced((current) => !current)}>{locale === "id" ? "Kontrol engine lanjutan" : "Advanced engine controls"}</button>{showAdvanced ? <div className="mt-3"><AccelerationCenter client={client} locale={locale} projectId={detail.project.id} currentSelection={detail.acceleration} compact onSelectionChanged={onAccelerationChanged} /></div> : null}</div><div><div className="mb-3 flex items-center justify-between"><p className="studio-label">{t.selectSubject}</p><UserFocus size={17} className="text-[#f5662c]" /></div>{tracks.length ? <div className="space-y-2">{tracks.map((track) => <button key={track.id} onClick={() => onPatch({ selected_face_track_id: track.id })} className={`track-option ${clip.selected_face_track_id === track.id ? "is-selected" : ""}`}><span><span className="block text-sm">{track.label}</span><span className="font-mono text-[10px] text-stone-500">{Math.round(track.confidence * 100)}% CONF.</span></span>{clip.selected_face_track_id === track.id ? <CheckCircle size={18} className="text-[#f5662c]" /> : <CaretDown size={15} className="-rotate-90 text-stone-600" />}</button>)}</div> : <p className="border-l border-[#f5662c] pl-3 text-sm leading-6 text-stone-500">{t.noTracks}</p>}</div><div className="border-y border-white/8 py-5"><div className="mb-3 flex items-center justify-between"><p className="studio-label">{t.gaps}</p><span className="font-mono text-[10px] text-stone-600">{gaps.length}</span></div><p className="text-sm leading-6 text-stone-500">{gaps.length ? "Hold, then ease toward center at marked intervals." : "No persisted subject loss."}</p></div>{previewStale ? <p role="status" className="preview-stale"><WarningCircle size={16} />{t.previewStale}</p> : null}<div className="space-y-2"><button className="action-primary w-full" disabled={busy || !health?.face_tracking.available} onClick={onPreview}><Play size={17} weight="fill" />{tracks.length && !clip.selected_face_track_id ? t.selectSubject : t.makePreview}</button><button className="action-secondary w-full justify-center" disabled={busy || previewStale || clip.status !== "preview_ready"} onClick={onApprove}><CheckCircle size={17} />{t.approve}</button><button className="action-export w-full" disabled={busy || previewStale || clip.status !== "approved"} onClick={onExport}><ShareFat size={17} weight="fill" />{t.export}</button></div>{job?.error ? <p className="text-xs text-red-300">{job.error}</p> : null}<div><p className="studio-label">{t.exportHistory}</p>{exports.length ? exports.slice(-3).reverse().map((artifact) => <a className="export-history-item" key={artifact.id} href={`/api/artifacts/${artifact.id}`}><FolderOpen size={16} /><span>MP4 export</span>{artifactMetadataValue(artifact, "encoder") ? <code>{artifactMetadataValue(artifact, "encoder")}</code> : null}{artifactMetadataValue(artifact, "tracker_engine") ? <code>{artifactMetadataValue(artifact, "tracker_engine")}</code> : null}</a>) : <p className="text-sm text-stone-600">—</p>}</div></div> : <div className="p-5 text-sm leading-6 text-stone-500">{t.noProjectBody}</div>}</aside>;
}

function HealthDialog({ health, t }: { health: RuntimeHealth | null; t: AppCopy }) {
...
function artifactMetadataValue(artifact: Artifact, key: string) { const value = artifact.metadata?.[key]; return typeof value === "string" ? value : null; }
function previewMatchesSelection(detail: ProjectDetail, clipId: string) {
  const clip = detail.clips.find((item) => item.id === clipId);
  const selection = detail.acceleration;
  const resolution = clip?.tracking_resolution;
  const preview = latestArtifact(detail.artifacts, clipId, "tracking_preview");
  if (!clip || clip.status !== "preview_ready" || !selection || !resolution || !preview) return false;
  const trackerMatches = selection.tracker_engine === "auto" || selection.tracker_engine === resolution.tracker_engine;
  const renderedEncoder = artifactMetadataValue(preview, "encoder_mode");
  const encoderMatches = selection.encoder_mode === "auto" || selection.encoder_mode === renderedEncoder;
  return trackerMatches && encoderMatches;
}
