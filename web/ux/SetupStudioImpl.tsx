import {
  ArrowClockwise, CheckCircle, CircleNotch, Cpu, FilmReel, GearSix, Lightning,
  LockSimple, Play, ShieldCheck, Sparkle, TerminalWindow, WarningCircle,
} from "@phosphor-icons/react";
import { useCallback, useEffect, useMemo, useState } from "react";
import type { SetupClient, SetupComponent, SetupJob, SetupStatus } from "./SetupStudio";
...
const copy = {
  id: {
    ...
    component: "Komponen", engine: "Mesin", steps: ["Siapkan mesin", "Impor video", "Analisis klip", "Kunci subjek", "Setujui dan ekspor"],
  ...
  en: {
    ...
    component: "Component", engine: "Engine", steps: ["Set up engine", "Import a video", "Analyze clips", "Lock a subject", "Approve and export"],
...
export function SetupStudio({ client = setupApi, onEnterStudio }: { client?: SetupClient; onEnterStudio?: () => void }) {
  ...
  const beginInstall = useCallback(async (component: string) => {
    ...
    try {
      ...
      setBusyComponent(null); setIssue(error instanceof Error ? error.message : "Installer could not start");
    }
  }, [client, refresh]);
  const missingRequired = useMemo(() => status?.components.filter((component) => component.required && component.state !== "ready") ?? [], [status]);
  ...
  return <main className="setup-shell">
    ...
    <div className="setup-layout">
      <section className="setup-main">
        <div className="eyebrow"><Sparkle size={15} />{t.setup}</div><h1>{t.title}</h1><p className="setup-lede">{t.body}</p>
        ...
        <section className="tutorial-card" aria-labelledby="tutorial-heading"><div><p className="section-kicker">{t.welcome}</p><h2 id="tutorial-heading">{t.tutorialTitle}</h2><p>{t.tutorialBody}</p></div><ol className="step-list">{t.steps.map((step, index) => <li key={step}>{index + 1}. {step}</li>)}</ol></section>
        <section aria-label={t.component} className="components-section"><div className="section-heading"><div><p className="section-kicker">{t.engine}</p><h2>Local engine check</h2></div><span>{status?.components.length ?? 0} checks</span></div><div className="component-grid">{status?.components.map((component) => <ComponentCard key={component.id} component={component} locale={locale} t={t} busy={busyComponent === component.id} onInstall={beginInstall} />)}{status?.hardware.adapter && !status.hardware.gpu_ready ? <GpuCard hardware={status.hardware} t={t} busy={busyComponent === "whisper_gpu"} onInstall={beginInstall} /> : null}</div></section>
      </section>
      <aside className="setup-aside"><section className="hardware-card"><div className="panel-heading"><Cpu size={19} /><span>{t.hardware}</span></div><p className="adapter">{status?.hardware.adapter ?? t.notDetected}</p><dl><div><dt>{t.detected}</dt><dd>{status?.hardware.driver ? `Driver ${status.hardware.driver}` : "—"}</dd></div><div><dt>Whisper</dt><dd>{status?.hardware.gpu_ready ? t.gpu : t.cpu}</dd></div><div><dt>Face tracking</dt><dd>{t.cpu}</dd></div></dl></section><section className="safety-card"><ShieldCheck size={20} /><div><h2>{t.safeTitle}</h2><p>{t.safeBody}</p></div></section>{job ? <section className="job-card"><div className="job-title"><CircleNotch className={job.stage === "completed" ? "" : "spin"} size={17} /><span>{t.progress}</span></div><p>{job.message}</p><div className="job-meter"><span style={{ width: `${Math.round(job.progress * 100)}%` }} /></div></section> : null}<button className="enter-button" onClick={onEnterStudio} disabled={!status?.is_ready || busyComponent !== null}><Play size={17} weight="fill" />{t.openStudio}</button>{!status?.is_ready ? <p className="enter-help"><WarningCircle size={15} />{t.enterBlocked}</p> : null}</aside>
    </div>{issue ? <div role="alert" className="setup-alert"><WarningCircle size={18} />{issue}</div> : null}
  </main>;
}
...
function ComponentCard({ component, locale, t, busy, onInstall }: { component: SetupComponent; locale: Locale; t: typeof copy.id; busy: boolean; onInstall: (component: string) => void }) {
  const stateLabel = component.state === "ready" ? t.installed : component.state === "installing" ? t.installing : component.state === "failed" ? t.failed : t.missing;
  const displayLabel = component.id === "face_tracking" ? "Face tracking" : component.label;
  const button = locale === "id" ? `Pasang ${component.label}` : `${t.install} ${component.label}`;
  return <article className={`component-card is-${component.state}`}><div className="component-top"><span className="component-icon">{component.state === "ready" ? <CheckCircle size={20} weight="fill" /> : <WarningCircle size={20} weight="fill" />}</span><span className="status-word">{stateLabel}</span></div><h3>{displayLabel}{component.acceleration ? <span> · {component.acceleration.toUpperCase()}</span> : null}</h3><p>{component.detail}</p><div className="component-footer"><small>{component.version ?? "—"}</small>{component.state !== "ready" && installable.has(component.id) ? <button onClick={() => onInstall(component.id)} disabled={busy}><TerminalWindow size={15} />{busy ? t.installing : button}</button> : null}</div></article>;
}

function GpuCard({ hardware, t, busy, onInstall }: { hardware: SetupStatus["hardware"]; t: typeof copy.id; busy: boolean; onInstall: (component: string) => void }) {
